...
const ImgVert = ({ texts, imgSrc, imgAlt, height = '60' }: Props) => {
  ...
  return (
    <>
      ...
      <div className="pl-16">
        <div className="prose mt-3 max-w-none text-2xl text-stone-500 [text-orientation:upright] [writing-mode:vertical-rl] dark:prose-invert dark:text-amber-300 xl:col-span-2">
          {texts.map((t) => (
            <p key={t}> {t} </p>
          ))}
        </div>
